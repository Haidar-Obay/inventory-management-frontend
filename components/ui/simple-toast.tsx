...
export function SimpleToastProvider({ children }: { children: React.ReactNode }) {
  ...
  const timeoutsRef = useRef<Map<string, { timeoutId: NodeJS.Timeout; startTime: number }>>(new Map());
  ...
  return (
    <ToastContext.Provider value={{ addToast, removeToast, pauseToast, resumeToast, toasts, startRemoveToast }}>
      {children}
...
// Toast component
function SimpleToastItem({ toast, onClose }: { toast: Toast; onClose: () => void }) {
  ...
  const animationStyles = {
    ...
    transform: toast.isExiting ? 'translateY(16px)' : visible ? 'translateY(0)' : 'translateY(16px)',
    ...
    >
      <div className="flex w-full items-start">
        ...
        <div className="flex-1 mr-2">
          ...
          {toast.description && <div className="text-sm mt-0.5 opacity-90 font-normal leading-snug">{toast.description}</div>}
        </div>
...
const internalToast = {
  ...
  error: (options: { title: string; description?: string; duration?: number }) => {
    // This implementation will be replaced at runtime
  },
