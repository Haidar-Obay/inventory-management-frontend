...
const AttachmentsSection = React.memo(({ 
  formData, 
  onFormDataChange, 
  t
}) => {
  ...
  const handleFileUpload = useCallback((files) => {
    ...
    setTimeout(() => {
      onFormDataChange('attachments', [...attachments, ...newAttachments]);
      setUploadProgress(0);
    }, 500);
  }, [attachments, onFormDataChange]);

  const handleRemoveAttachment = useCallback((index) => {
    const updated = [...attachments];
    updated.splice(index, 1);
    onFormDataChange('attachments', updated);
  }, [attachments, onFormDataChange]);
    ...
    <Box sx={{ mt: 4 }}>
      ...
      <Box 
        sx={{ 
          ...
          p: 3,
          textAlign: 'center',
          bgcolor: dragActive ? 'primary.light' : 'rgb(249 250 251)',
