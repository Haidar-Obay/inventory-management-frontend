  ...
  ({
        ...
        if (!gridInitializedRef.current) {
          const sourceItems =
            Array.isArray(formData.items) && formData.items.length > 0
              ? formData.items
              : Array.isArray(formData.gridData)
                ? formData.gridData
                : [];
    ...
    function isDataChanged() {
      ...
      if (!isEdit) {
        ...
        const hasGridData = gridData.some(
          (row) =>
            row.line ||
            row.itemcode ||
            row.item_id ||
      ...
      const gridDataChanged =
        JSON.stringify(currentGridData) !== JSON.stringify(originalGridData);
    ...
    const handleSave = useCallback(async () => {
      if (isEdit && !isDataChanged()) {
        addToast({
          ...
          title: tToast("error"),
          description:
            t("customers.management.noChangesDesc") ||
      ...
      try {
        ...
        const dataToSave = {
          ...
          items: (Array.isArray(gridData) ? gridData : [])
            .map((r) => ({
              item_id:
                r.item_id ??
                (typeof r.itemcode === "number"
                  ? r.itemcode
                  : typeof r.itemcode === "string" &&
                      r.itemcode !== "" &&
                      !isNaN(Number(r.itemcode))
                    ? Number(r.itemcode)
                    : null),
              itemname: r.itemname ?? "",
              price:
        ...
        if (response && response.status) {
          ...
          addToast({
            ...
            description:
              response?.message ||
              tToast(isEdit ? "updateError" : "createError"),
            duration: 3000,
          });
        }
      } catch (error) {
        addToast({
          ...
          title: tToast("error"),
          description:
            error.message || tToast(isEdit ? "updateError" : "createError"),
          duration: 3000,
        });
      } finally {
                ...
                <Grid container spacing={1.5}>
                  ...
                  <Grid item xs={12} sm={6} md={3}>
                    ...
                    <RTLTextField
                      ...
                      onChange={(e) =>
                        handleFormDataChange("valid_from", e.target.value)
                      }
                      InputLabelProps={{ shrink: true }}
                    />
                  </Grid>
                  <Grid item xs={12} sm={6} md={3}>
                    ...
                    <RTLTextField
                      ...
                      onChange={(e) =>
                        handleFormDataChange("valid_till", e.target.value)
                      }
                      InputLabelProps={{ shrink: true }}
                    />
                  </Grid>
