...
export const config = {
  // API Configuration
  API_BASE_URL: process.env.NEXT_PUBLIC_API_URL || (process.env.NODE_ENV === 'development' ? 'http://localhost:8000' : 'http://binbothub.com/backend'),
...
export const getTenantApiUrl = (tenantName, endpoint = '') => {
  ...
  // Production: ensure /backend path is included for proper routing
  return `http://${tenantName}.${config.CENTRAL_DOMAIN}/backend/${endpoint}`;
};
